...
export default function App() {
  const [activeTab, setActiveTab] = useState<'pos' | 'crm' | 'erp'>('pos');
  const [products, setProducts] = useState<Product[]>(INITIAL_PRODUCTS);
  const [leads, setLeads] = useState<Lead[]>(INITIAL_LEADS);
  const [cart, setCart] = useState<CartItem[]>([]);
  const [searchQuery, setSearchQuery] = useState('');
  const [selectedCategory, setSelectedCategory] = useState<string>('All');
  const [strapiStatus, setStrapiStatus] = useState<{ connected: boolean; latency: number }>({ connected: false, latency: 0 });
  const [showReceiptModal, setShowReceiptModal] = useState(false);
  const [lastOrder, setLastOrder] = useState<{ items: CartItem[]; total: number; orderId: string } | null>(null);
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-between items-center bg-slate-900/60 border border-slate-800 p-4 rounded-2xl backdrop-blur-sm">
                <div className="relative w-full sm:w-72">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-950 border border-slate-800 rounded-xl pl-9 pr-4 py-2 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500 transition-colors"
                  />
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {filteredProducts.map((p) => (
                  <div
                    key={p.id}
                    className="bg-slate-900 border border-slate-800 hover:border-indigo-500/50 rounded-2xl p-4 flex flex-col justify-between transition-all duration-200 hover:shadow-xl hover:shadow-indigo-500/5 group"
                  >
                    ...
                    <button
                      onClick={() => addToCart(p)}
                      disabled={p.stock === 0}
                      className="mt-4 w-full py-2 bg-indigo-600 hover:bg-indigo-500 disabled:bg-slate-800 disabled:text-slate-600 text-white rounded-xl font-medium text-xs flex items-center justify-center space-x-2 transition-all shadow-md shadow-indigo-600/20"
                    >
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 flex flex-col justify-between shadow-2xl h-[calc(100vh-140px)] sticky top-24">
                      ...
                      <div key={item.product.id} className="py-3 flex items-center justify-between">
                        ...
                        <div className="flex items-center space-x-2">
                          <button
                            ...
                            className="w-6 h-6 rounded-md bg-slate-800 hover:bg-slate-700 text-slate-200 flex items-center justify-center text-xs font-bold"
                          >
                            -
                          </button>
              ...
              <div className="border-t border-slate-800 pt-4 space-y-2">
                ...
                <button
                  ...
                  className="w-full py-3 mt-4 bg-gradient-to-r from-emerald-600 to-teal-500 hover:from-emerald-500 hover:to-teal-400 disabled:from-slate-800 disabled:to-slate-800 text-white font-bold rounded-xl text-sm flex items-center justify-center space-x-2 transition-all shadow-lg shadow-emerald-600/20"
                >
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              {(['New', 'Contacted', 'Proposal', 'Won'] as const).map((stage) => {
                ...
                return (
                  <div key={stage} className="bg-slate-900/60 border border-slate-800/80 rounded-2xl p-4 flex flex-col h-[520px]">
                    ...
                    <div className="space-y-3 overflow-y-auto flex-1 pr-1">
                      {stageLeads.map((lead) => (
                        <div
                          key={lead.id}
                          className="bg-slate-900 border border-slate-800 hover:border-indigo-500/40 rounded-xl p-3 shadow-md space-y-2 group"
                        >
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl max-w-md w-full p-6 space-y-4 shadow-2xl relative">
            ...
            <div className="flex space-x-3 pt-2">
              <button
                ...
                className="flex-1 py-2.5 bg-slate-800 hover:bg-slate-700 text-white rounded-xl text-xs font-semibold flex items-center justify-center space-x-2 transition-all"
              >
              ...
              <button
                onClick={() => setShowReceiptModal(false)}
                className="flex-1 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-xs font-semibold flex items-center justify-center space-x-2 transition-all"
              >
                <span>Done</span>
              </button>
            </div>
