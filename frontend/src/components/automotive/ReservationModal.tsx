        ...
        <div className={`flex items-center justify-between border-b pb-4 ${
          ...
          <div>
            ...
            <h2 className={`text-lg font-bold mt-1 ${isLight ? 'text-slate-900' : 'text-white'}`}>
              Import / Reserve Deposit Holding
            </h2>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                <label className={`block text-[10px] font-bold uppercase tracking-wider mb-1 ${
                  isLight ? 'text-slate-700' : 'text-neutral-400'
                }`}>HOLDING DEPOSIT AMOUNT (KES)</label>
                <input
                  type="text"
                  required
                  value={form.depositAmount}
                  onChange={(e) => setForm({ ...form, depositAmount: e.target.value })}
                  className={`w-full border rounded-xl px-3 py-2.5 text-xs outline-none font-bold ${
                    isLight
                      ? 'bg-slate-100 border-slate-300 text-slate-900 focus:bg-white focus:border-amber-600'
                      : 'bg-[#121622] border-[#1e2638] text-[#c9a84c] focus:border-[#c9a84c]'
                  }`}
                />
              </div>
              ...
              <div>
                ...
                >
                  <option value="M-Pesa Express / Paybill">M-Pesa Express / Paybill</option>
                  <option value="Bank Wire Transfer (RTGS)">Bank Wire Transfer (RTGS)</option>
            ...
            <div>
              <label className={`block text-[10px] font-bold uppercase tracking-wider mb-1 ${
                isLight ? 'text-slate-700' : 'text-neutral-400'
              }`}>SPECIAL INSTRUCTIONS / IMPORT PREFERENCES</label>
              <textarea
                rows={2}
                value={form.notes}
                onChange={(e) => setForm({ ...form, notes: e.target.value })}
                placeholder="Mention exterior color, trim options, or target delivery date..."
            ...
            >
              <Send size={14} />
              <span>{loading ? 'Securing Deposit...' : 'CONFIRM RESERVATION DEPOSIT'}</span>
            </button>
          </form>
        )}
      </div>
