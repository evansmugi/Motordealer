...
import { ArrowRight, ShieldCheck, Zap, Sparkles, Cpu, Layers, Brain } from 'lucide-react';
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '60px' }}>
      ...
      <section style={{ maxWidth: '1280px', margin: '0 auto', width: '100%', padding: '0 40px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '32px' }}>
          <div>
            ...
            <h2 style={{ fontSize: '32px', fontWeight: '900', color: '#F8FAFC' }}>
              Curated Technology Matrix
            </h2>
          </div>
          <Link href="/products" style={{ color: '#60A5FA', fontSize: '14px', fontWeight: '700', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '6px' }}>
          ...
          {CATEGORIES.map((cat: CategoryItem) => (
            <div
              ...
              style={{
                ...
                padding: '24px',
                cursor: 'pointer',
                border: selectedCat === cat.slug ? '1px solid #3B82F6' : '1px solid rgba(255,255,255,0.08)',
                background: selectedCat === cat.slug ? 'rgba(59, 130, 246, 0.1)' : 'rgba(14, 16, 23, 0.7)',
                transition: 'all 0.3s ease'
              }}
            >
              <div style={{ width: '40px', height: '40px', borderRadius: '10px', background: 'rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '16px', color: '#3B82F6' }}>
                <Cpu size={22} />
              </div>
              <h3 style={{ fontSize: '18px', fontWeight: '800', color: '#F8FAFC', marginBottom: '6px' }}>{cat.name}</h3>
              <p style={{ fontSize: '12px', color: '#94A3B8', lineHeight: '1.5' }}>{cat.description}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section style={{ maxWidth: '1280px', margin: '0 auto', width: '100%', padding: '0 40px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px' }}>
          <div>
            <h2 style={{ fontSize: '28px', fontWeight: '900', color: '#F8FAFC' }}>
          ...
          {selectedCat !== 'ALL' && (
            <button onClick={() => setSelectedCat('ALL')} style={{ background: 'rgba(255,255,255,0.05)', border: 'none', borderRadius: '8px', padding: '6px 14px', color: '#94A3B8', fontSize: '12px', cursor: 'pointer' }}>
              Reset Filter
            </button>
          )}
        </div>
      ...
      <section style={{ maxWidth: '1280px', margin: '0 auto', width: '100%', padding: '0 40px' }}>
        <div className="glass-panel" style={{ borderRadius: '24px', padding: '60px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '60px', alignItems: 'center' }}>
          <div>
            ...
            <h2 style={{ fontSize: '36px', fontWeight: '900', color: '#F8FAFC', margin: '12px 0 20px', lineHeight: '1.2' }}>
          ...
          <div style={{ position: 'relative' }}>
            <img
              ...
              style={{ width: '100%', height: '360px', objectFit: 'cover', borderRadius: '20px', border: '1px solid rgba(255,255,255,0.1)' }}
            />
          </div>
        </div>
      </section>
